  ...
  override def parse(): SecurityScheme = {
    ...
    ctx.link(node) match {
      ...
      case Right(value) =>
        ...
        AnnotationParser(scheme, map, List(VocabularyMappings.securityScheme))(WebApiAdapterShapeParserContext(ctx))
          .parse()
        ...
        entry.value.tagType match {
          case YType.Map =>
            ...
            value.key(
              "queryString",
              queryEntry => {
                Raml10TypeParser(queryEntry, shape => shape.adopted(scheme.id))(WebApiAdapterShapeParserContext(ctx))
                  .parse()
            ...
            AnnotationParser(scheme, value)(WebApiAdapterShapeParserContext(ctx)).parse()
          case YType.Null =>
          case _ =>
