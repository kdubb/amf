...
import amf.plugins.document.webapi.parser.spec.declaration.raml.{RamlJsonSchemaExpression, RamlXmlSchemaExpression}
...
import amf.plugins.domain.webapi.parser.spec.common.ShapeExtensionParser
import amf.plugins.domain.webapi.parser.spec.declaration.TypeInfo
import amf.plugins.domain.webapi.parser.{RamlTypeDefMatcher, TypeName, spec}
...
object Raml10TypeParser {
  ...
  def apply(entryOrNode: YMapEntryLike,
            ...
            defaultType: DefaultType)(implicit ctx: ShapeParserContext): Raml10TypeParser =
...
case class Raml10TypeParser(entryOrNode: YMapEntryLike,
  ...
  override def typeParserFactory: (YMapEntry, Shape => Unit, Boolean, DefaultType) => RamlTypeParser =
    (entry, adopt, isAnnotation, default) =>
...
object Raml08TypeParser {
  ...
  def apply(entryOrNode: YMapEntryLike,
            ...
            adopt: Shape => Unit,
            isAnnotation: Boolean,
            defaultType: DefaultType)(implicit ctx: ShapeParserContext): Raml08TypeParser =
...
case class Raml08TypeParser(entryOrNode: YMapEntryLike,
  ...
  override def typeParserFactory: (YMapEntry, Shape => Unit, Boolean, DefaultType) => RamlTypeParser =
    (entry, adopt, isAnnotation, default) => Raml08TypeParser(entry, adopt, isAnnotation, default)
  ...
  case class Raml08ReferenceParser(text: String, node: YNode, name: String)(implicit ctx: ShapeParserContext) {
    def parse(): Some[AnyShape] = {
      val shape: AnyShape = ctx.findType(text, SearchScope.All) match {
        case Some(s: AnyShape) =>
          s.link(AmfScalar(text), Annotations(node.value), Annotations.synthesized())
            .asInstanceOf[AnyShape]
            .withName(name, s.name.annotations())
        case None =>
  ...
  case class Raml08SchemaParser(map: YMap, adopt: Shape => Unit)(implicit ctx: ShapeParserContext) {
    def parse(): Option[AnyShape] = {
...
case class Raml08ExampleParser(s: AnyShape, map: YMap)(implicit ctx: ShapeParserContext) {
...
case class Raml08UnionTypeParser(shape: UnionShape, types: YSequence, ast: YPart)(implicit ctx: ShapeParserContext) {
...
sealed abstract class RamlTypeParser(entryOrNode: YMapEntryLike,
  ...
  def typeParserFactory: (YMapEntry, Shape => Unit, Boolean, DefaultType) => RamlTypeParser
  ...
  private def parseNilUnion() = {
    ...
    val parsed = node.value match {
      case s: YScalar =>
        ...
        typeParserFactory(toParse, shape => shape.withId(union.id), typeInfo.isAnnotation, defaultType).parse().get
      case m: YMap =>
        ...
        typeParserFactory(toParse, s => s.withId(union.id), typeInfo.isAnnotation, defaultType).parse().get
    }
  ...
  private def parseReference(node: YNode, parendId: String, createLink: AnyShape => Shape): Option[Shape] = {
    ctx.link(node) match {
      case Left(key) =>
        val referenced = ctx.findType(key,
      ...
      case _ =>
        val text       = node.as[YScalar].text
        val referenced = ctx.findType(text, SearchScope.Named)
  ...
  trait CommonScalarParsingLogic extends QuickFieldParsingOps {
          ...
          val inherits: Seq[AmfElement] = superTypes.zipWithIndex.map {
            case (node, i) =>
              ...
              node.as[YScalar].text match {
                ...
                case s =>
                  ctx.findType(s, SearchScope.All) match {
      ...
      val unresolvedShape = UnresolvedShape(
        ...
        fatherMap.map(m =>
          (resolvedKey: Option[String]) =>
            spec.common.ShapeExtensionParser(shape, m, ctx, typeInfo, overrideSyntax = resolvedKey)),
        Some((k: String) =>
  ...
  case class NodeShapeParser(shape: NodeShape, map: YMap) extends AnyShapeParser with SchemaUsageRestrictions {
    override def parse(): AnyShape = {
      ...
      map.key("additionalProperties".asRamlAnnotation).foreach { entry =>
        typeParserFactory(entry, s => s.adopted(shape.id), true, defaultType)
          .parse()
          .foreach { parsed =>
      ...
      map.key(
        ...
        entry => {
          Draft4ShapeDependenciesParser(shape, entry.value.as[YMap], shape.id, JSONSchemaDraft4SchemaVersion)(ctx)
            .parse()
        }
      )
  ...
  abstract class ShapeParser extends RamlTypeSyntax with QuickFieldParsingOps {
