...
import amf.plugins.document.webapi.parser.spec.emitter.ContentEmitters
...
case class OasScalarShapeEmitter(scalar: ScalarShape,
  ...
  override def emitters(): Seq[EntryEmitter] = {
    ...
    fs.entry(ScalarShapeModel.DataType)
      .foreach { f =>
        val typeDefStr = spec.matchType(typeDef.get)
    ...
    fs.entry(ScalarShapeModel.Format) match {
      ...
      case None =>
        spec.matchFormat(typeDef.getOrElse(UndefinedType)) match {
          case Some(format) =>
