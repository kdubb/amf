...
//          AnnotationParser.parseExtensions(parent, n.obj)
  ...
  implicit class FieldOps(field: Field)(implicit ctx: ParserErrorHandling) {
...
object RamlScalarNode extends SpecAnnotationCriteria {
  def apply(node: YNode)(implicit ctx: ParserErrorHandling): ScalarNode = {
    node.value match {
  ...
  private def createScalarValuedNode(obj: YMap)(implicit ctx: ParserErrorHandling): RamlScalarValuedNode = {
    ...
    if (values.nonEmpty) {
      values.tail.foreach(d => ctx.eh.violation(DuplicatePropertySpecification, "", s"Duplicated key 'value'.", d))
    }
  ...
  private def unexpected(key: YNode)(implicit ctx: ParserErrorHandling): Unit =
    ctx.eh.violation(UnexpectedRamlScalarKey,
                     "",
