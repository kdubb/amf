...
case class ContentParser(parsers: Seq[EntryParser[ScalarShape]]){
  ...
  def parse(scalar: ScalarShape, map: YMap)(implicit ctx: ShapeParserContext): Unit = parsers.foreach(_.parse(scalar, map))
}
...
private[this] case class ContentSchemaParser(adopt: Shape => Unit, version: SchemaVersion) extends EntryParser[ScalarShape]{
  override def parse(node: ScalarShape, map: YMap)(implicit ctx: ShapeParserContext): Unit = {
    map.key("contentSchema").foreach { entry =>
      OasTypeParser(entry, adopt, version)(ctx).parse().foreach { s =>
